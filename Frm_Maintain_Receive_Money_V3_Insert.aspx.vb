...
Public Class Frm_Maintain_Receive_Money_V3_Insert
    ...
    Private Sub btn_save_Click(sender As Object, e As EventArgs) Handles btn_save.Click
        ...
        If count_bg = 0 Then
            ...
            Try
                ...
                If dao_fee88.fields.feeabbr = "9005" Or dao_fee88.fields.feeabbr = "9006" Then
                    ...
                    For Each dao_fee_det.fields In dao_fee_det.datas
                        iii += 1
                        Steps = "3"
                        Dim dao_sp As New DAO_FDA_SPECIAL_PAYMENT.TB_SYSTEMS_PAYMENT_DETAIL
                        dao_sp.GetDataby_IDA(dao_fee_det.fields.fk_id)
                        With dao_sp.fields
                            .STATUS_ID = 8
                        ...
                        dao_sp.update()
                        Steps = "4"
                        Dim dao_log As New DAO_FDA_SPECIAL_PAYMENT.TB_SPD_LOG
                        With dao_log.fields
                            .CREATE_DATE = Date.Now
                            .FK_IDA = dao_fee_det.fields.fk_id
                            .process = dao_fee_det.fields.process_id
                            .type = 1
                        End With
                        dao_log.insert()

                        'Insert_log_error(ref01, ref02, iii & ". เลข REF : " & dao_fee_det.fields.lcnno_convert, dao_fee_det.fields.fk_id)
                    Next
            ...
            '    End Try
            '    If dao_fee_det.fields.process_id = "555555" Then
            '        citizen = dao_spay.fields.CITIZEN_REQUEST
            '    Else
            '        citizen = dao_pay.fields.CITIZEN_REQUEST
            '    End If
            ...
            If Request.QueryString("pre") = "" Then
                ...
                Else
                    url &= "&law=1"
                    'System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "window.open('" & url & "', '_blank'); window.open('" & url2 & "', '_blank'); parent.$('#ctl00_ContentPlaceHolder1_btnRedirect').click();", True)
                    System.Web.UI.ScriptManager.RegisterStartupScript(Page, GetType(Page), "ใส่ไรก็ได้", "window.open('" & url & "', '_blank'); parent.$('#ctl00_ContentPlaceHolder1_btnRedirect').click();", True)
