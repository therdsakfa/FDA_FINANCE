...
Public Class UC_Disburse_Budget_PayType_Pass_Check_Sign
    ...
    Private Sub rg_check_sign_NeedDataSource(sender As Object, e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rg_check_sign.NeedDataSource
        ...
        If is_rebill = True Then
            dt = bao.get_bill_check_sign_rebill_V2(bg_use, bgyear, stat - 1, g)
            dt.Columns.Add("stat")
        ...
        ElseIf is_no_rebill = True Then
            dt = bao.get_bill_check_sign_norebill(bg_use, bgyear, stat - 1, g)
            dt.Columns.Add("stat")
            For Each dr As DataRow In dt.Rows
                If dr("STATUS_ID") >= stat Then
                    dr("stat") = "เสนอเซ็นเช็คแล้ว"
            ...
            Next
        Else
            dt = bao.get_bill_check_sign_V3(bg_use, bgyear, stat - 1, g)
            dt.Columns.Add("stat")
            For Each dr As DataRow In dt.Rows
                If dr("STATUS_ID") >= stat Then
                    dr("stat") = "เสนอเซ็นเช็คแล้ว"
            ...
            If item("id_det").Text <> "" Then
                ...
                For i As Integer = 0 To arr_det.Length - 1
                    Try
                        ...
                        dao_det.fields.CHECK_SIGN_DATE = Date.Now
                        dao_det.update()
