...
Public Class UC_Disburse_Bill_Status
    ...
    Private Sub rgBill_status_Init(sender As Object, e As EventArgs) Handles rgBill_status.Init
        Dim rg_Debtor As RadGrid = rgBill_status
        Dim Rad_Utility As New Radgrid_Utility
        Rad_Utility.Rad = rgBill_status
    ...
    Private Sub rgBill_status_NeedDataSource(sender As Object, e As GridNeedDataSourceEventArgs) Handles rgBill_status.NeedDataSource
        If search_type = 1 Then
            If bill_number <> "" Then
                Dim bao As New BAO_BUDGET.DISBURSE_BUDGET
                Dim dt As DataTable = bao.get_bill_by_bill_number(bill_number, bgyear)
                dt.Columns.Add("status")
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        Dim uti_stat As New Utility_other
                        uti_stat.return_status_bill(dr("BUDGET_DISBURSE_BILL_ID"))
                        dr("status") = uti_stat.TextStatus
                    Next
                End If

                rgBill_status.DataSource = dt
        ...
        ElseIf search_type = 2 Then
            If doc_number <> "" Then
                Dim bao As New BAO_BUDGET.DISBURSE_BUDGET
                Dim dt As DataTable = bao.get_bill_by_doc_number(doc_number, bgyear)
                dt.Columns.Add("status")
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        Dim uti_stat As New Utility_other
        ...
        ElseIf search_type = 3 Then
            If cus_name <> "" Then
                Dim bao As New BAO_BUDGET.DISBURSE_BUDGET
                Dim dt As DataTable = bao.get_bill_by_cus_name(cus_name, bgyear)
                dt.Columns.Add("status")
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        Dim uti_stat As New Utility_other
        ...
        ElseIf search_type = 4 Then
            If cus_name <> "" Then
                ...
                If equal_t = "=" Then
                    dt = bao.get_bill_by_Amount_Equal(cus_name, bgyear)
                ElseIf equal_t = ">" Then
                    dt = bao.get_bill_by_Amount_More_Than(cus_name, bgyear)
                ElseIf equal_t = "<" Then
                    dt = bao.get_bill_by_Amount_Less_Than(cus_name, bgyear)
                End If
        ...
        ElseIf search_type = 5 Then
            If K_nymber <> "" Then
                Dim bao As New BAO_BUDGET.DISBURSE_BUDGET
                Dim dt As DataTable = bao.get_bill_by_k_number(K_nymber, bgyear)
                dt.Columns.Add("status")
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        Dim uti_stat As New Utility_other
        ...
        ElseIf search_type = 6 Then
            If K_nymber <> "" Then
                Dim bao As New BAO_BUDGET.DISBURSE_BUDGET
                Dim dt As DataTable = bao.get_bill_by_c_number(K_nymber, bgyear)
                dt.Columns.Add("status")
                If dt.Rows.Count > 0 Then
                    For Each dr As DataRow In dt.Rows
                        Dim uti_stat As New Utility_other
    ...
    Public Sub rg_rebind()
        rgBill_status.Rebind()
    End Sub

    Private Sub rgBill_status_SelectedIndexChanged(sender As Object, e As EventArgs) Handles rgBill_status.SelectedIndexChanged
        Dim item As GridDataItem = DirectCast(rgBill_status.SelectedItems(0), GridDataItem)
        Dim id As String = item("BUDGET_DISBURSE_BILL_ID").Text
        Dim uti_stat As New Utility_other
        Dim id_stat As Integer = 0
        id_stat = uti_stat.return_status_bill(id)
        Dim uti_get_url As New Utility_other
        Dim url As String = uti_get_url.get_url_bill(id_stat, id)
